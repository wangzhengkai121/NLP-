...
def create_data(data, tokenizer, max_len):
    ...
    for content in data:
        ...
        features = {'input_ids': text_ids,
                    'attention_mask': [1] * len(text_ids),#会生成一个 全1列表，长度与 text_ids 相同
                    'raw_data': content}#注意力掩码告诉模型哪些 token 是真实的输入，哪些是填充的（padding）
        if title:
...
def default_collate(batch):#自定义批处理函数，将数据转换为Tensor
    """组batch
...
def plot_rouge_line(rouge_scores, title="ROUGE Scores", save_path=r'C:\Users\30505\Desktop\textRank\zhaiyao\NLP_2\picture\1_.png'):

    """绘制单篇文本的ROUGE指标折线图"""

    metrics = ['ROUGE-1', 'ROUGE-2', 'ROUGE-L']
    scores = [rouge_scores['rouge-1'], rouge_scores['rouge-2'], rouge_scores['rouge-l']]
    print(123)
...
def init_argument():
    ...
    parser.add_argument('--result_file', default='./data/predict_chouqu.tsv')
