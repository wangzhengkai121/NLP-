...
class WordSegmentation(object):
    ...
    def __init__(self, stop_words_file=None, allow_speech_tags=util.allow_speech_tags):
        ...
        allow_speech_tags = [util.as_text(item) for item in allow_speech_tags]  # 词性表
    ...
    def segment(self, text, lower=True, use_stop_words=True, use_speech_tags_filter=False):  # 进行分词
        ...
        jieba_result = pseg.cut(text)  # 对分词的词性进行标注

        if use_speech_tags_filter == True:  # 按照词性去除没有含义的词
            jieba_result = [w for w in jieba_result if w.flag in self.default_speech_tag_filter]
        else:
            jieba_result = [w for w in jieba_result]

        # 去除特殊符号
        word_list = [w.word.strip() for w in jieba_result if w.flag != 'x']  # x代表符号和未知数,标点符号
        word_list = [word for word in word_list if len(word) > 0]

        if lower:
            word_list = [word.lower() for word in word_list]  # 转换成小写字符

        if use_stop_words:  # 去除停用词表中的词
...
class SentenceSegmentation(object):
    ...
    def __init__(self, delimiters=util.sentence_delimiters):  # 句子分隔符
        """
        Keyword arguments:
...
class Segmentation(object):
    ...
    def segment(self, text, lower=False):
        ...
        words_no_filter = self.ws.segment_sentences(sentences=sentences,
                                                    # 对sentences中每个句子分词仅去除了标点。而得到两级列表e.g[[你好/世界]，[你好/你好]]
                                                    lower=lower,
                                                    use_stop_words=False,
                                                    use_speech_tags_filter=False)
        # 间/酒店/位于/北京/东三环/里面/摆放/很多/雕塑/文艺/气息/十足\n答谢/宴于/晚上/8/点
        words_no_stop_words = self.ws.segment_sentences(sentences=sentences,  # 去除停用词表中的词后的两级列表
                                                        lower=lower,
                                                        use_stop_words=True,
                                                        use_speech_tags_filter=False)
        # 酒店/位于/北京/东三环/摆放/雕塑/文艺/气息\n答谢/宴于/晚上
        words_all_filters = self.ws.segment_sentences(sentences=sentences,  # 去除停用词表中的词和按照词性去除对应的词后的两级列表
                                                      lower=lower,
                                                      use_stop_words=True,
                                                      use_speech_tags_filter=True)

        return util.AttrDict(  # 允许通过点号（.）语法访问字典中的键值的类
            sentences=sentences,
